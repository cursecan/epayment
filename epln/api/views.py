...
from epln.models import Product, Transaksi, ResponseTransaksi, TransaksiRb, ResponseTransaksiRb
...
# TopUP Pln Rajabiller
class TopupTokenListrik_RajaBillerView(APIView):
    def get(self, request, format=None):
        return Response({'forbidend': 1},status=HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, format=None):
        ...
        # cek duplikasi transaksi
        trx_recek = TransaksiRb.objects.filter(idpel=user_pln, 
            product=produk_obj, timestamp__date=date.today(), status=0,
            request_type='p'
        )
        if trx_recek.exists():
        ...
        if user_obj.profile.agen or user_obj.profile.saldo - produk_obj.price >= user_obj.profile.limit:
            trx_obj_1 = TransaksiRb.objects.create(
                user = user_obj, price = produk_obj.price, 
                product=produk_obj, phone=phone,
                idpel = user_pln, nominal = produk_obj.nominal
            )

            trx_obj_1.refresh_from_db()

            if trx_obj_1.responsetransaksirb.status == '00':
                # jika inquery berhasil, lanjut payment
                trx_obj_2 = TransaksiRb.objects.create(
                    user = user_obj, price = produk_obj.price, 
                    product=produk_obj, phone=phone,
                    idpel = user_pln, nominal = produk_obj.nominal,
                    request_type='p', ref_inquery=trx_obj_1
                )
                ...
                result['price'] = produk_obj.price
                result['saldo'] = user_obj.profile.saldo
                result['account_num'] = trx_obj_2.idpel
                result['phone'] = trx_obj_2.phone
                result['struk'] = trx_obj_2.struk
